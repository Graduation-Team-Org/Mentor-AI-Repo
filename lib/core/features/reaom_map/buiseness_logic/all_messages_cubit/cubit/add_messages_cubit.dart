...
class AllMessagesCubit extends Cubit<AllMessagesState> {
  ...
  Future<void> addmessage({required String content, required BuildContext context}) async {
    try {
    ...
    try {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        if (assistantMessages.length > messages.length) {
          ...
          if (messages.length >= 2) { // At least one user message and one response
            currentSessionId = await ChatSessionService.saveChatSession(messages);
          }
