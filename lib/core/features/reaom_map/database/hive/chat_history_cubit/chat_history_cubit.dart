...
import '../models/history/chat_session_model.dart';
...
class ChatHistoryCubit extends Cubit<ChatHistoryState> {
  ...
  Future<void> saveChatSession(List<ChatMessageModel> messages,
      {String? title}) async {
    try {
      emit(ChatHistoryLoading());
      ...
      final sessionTitle = title ??
          (messages.isNotEmpty
              ? messages.first.content.substring(
                  0,
                  messages.first.content.length > 30
                      ? 30
                      : messages.first.content.length)
              : 'New Chat');
